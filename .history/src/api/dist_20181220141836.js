...
const getStrClass = (cls, def) => {
  ...
  let class_def = className && def && className + '--' + def || className || def || ''
  let class_defKh = className && def && className + '(' + def + ')' || className || def || ''
  return [className, def, class_def, class_defKh]
}
