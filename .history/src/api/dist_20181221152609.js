...
// 班级状态显示转换
const formatClsStatus = (props, userOrNo) => {
  let status = '异常'
...
const getStrClass = (cls, def) => {
  ...
  let classDef = (className && def && className + '--' + def) || className || def || ''
  let classDefKh = (className && def && className + '(' + def + ')') || className || def || ''
  return [className, def, classDef, classDefKh]
}
// 处理用户名和用户昵称数据
