...
function handleStatis (data) {
  ...
  for (let dataOne of data) {
    ...
    for (let [dataKey, dataValue] of Object.entries(dataOne)) {
      dataArrObj[dataKey] = (dataValue || dataValue === 0) ? dataValue : ''
    }
...
function handleChartData (data, whichEle, whichMoney) {
  ...
  let seriesName = {
    ...
    'datap': ['hwcounts', 'avgreadrate', 'maxreadrate', 'avgsubmitrate', 'maxsubmitrate']
  }
  let Xtime = {
    'hw': 'pubtime',
    'hws': 'hw_id',
  ...
  chartObj.formatter = function (e) {
    ...
    return totalTip[whichEle] + this.x + '<br>' + (whichEle === 'allI4' ? '变更时间：' + data[this.point.index].in_time + '<br>' : '') + this.series.name + '：' + this.y.toFixed(2) * 1 + unitTitle
  }
  return chartObj
}
export {
