    ...
    <v-flex>
      <v-toolbar color="grey darken-1" dark>
        ...
        <v-text-field
          ...
          hide-details
          clearable
          @click:append="getspacelist"
          @keyup.enter="getspacelist"
        >
        </v-text-field>
      </v-toolbar>
    </v-flex>
...
export default {
  ...
  methods: {
    ...
    getspacelist (oldOrNo) {
      if (!oldOrNo && (!this.search || isNaN(this.search * 1))) {
        this.$emit('update:setVsnackbar', [true, '请输入正确的用户ID', 'error'])
        return false
      }
      this.loading = true
      !oldOrNo && (this.news = 'news')
      // let dataObj = {'_id': this.pagination.page, class_id: this.search}
      axios('https://wxcs.ljlx.com/csm.ashx?method=spacedetail&_id=1&class_id=10779', '', {}, cb => {
        this.logining = false
        if (cb.code === 0) {
          console.log(cb.data)
          this.loading = false
