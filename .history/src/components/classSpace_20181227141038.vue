...
<div>
  <v-layout column wrap>
    ...
    <!-- 动态列表 -->
    <v-flex>
      <dataTable :desserts="desserts" which="classSpace0" :loading="loading" @getNew="getNew"
        :news="news" :setNew.sync="news" :setDrawer.sync="drawer" :setItem.sync="items">
      </dataTable> 
    </v-flex>
    <!-- 动态详情 -->
    <v-navigation-drawer
                  ...
                  <v-layout row wrap>
                    ...
                    >
                      <v-card flat tile class="d-flex">
                        <Dialog which="imageLook" media="media" v-if="item[0] === 1" :imageArr="['', '', 400, item[1][1]]"></Dialog>
                        <Dialog which="videoLook" media="media" v-if="item[0] === 2" :imageArr="['视频', item[1][0], 400, item[1][1]]"></Dialog>
                        <Dialog which="recordLook" media="media" v-if="item[0] === 3" :imageArr="['录音', item[1][0], 400, noRecord]"></Dialog>
                      </v-card>
                    </v-flex>
                  </v-layout>
                </v-container>
