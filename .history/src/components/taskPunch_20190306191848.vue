<template>
  <div>
    <v-layout class="layout" style="flex-wrap: wrap;align-items:center;margin: 10px 0;">
      <v-layout style="align-items:center;flex:0 0 auto;fontSize:16px;marginLeft:10px">
        按打卡任务名称查询:
      ...
      <v-layout style="align-items:center;flex:0 0 auto;fontSize:16px;marginLeft:10px">
        按打卡任务ID查询:
        <textField ref="teachId" label="" placeholder="请输入打卡任务ID" which="teacherId" @searchClass = "searchClass" :setModel.sync="teacherId" v-show="this.buttonIndex"> </textField>
      </v-layout>
    </v-layout>
