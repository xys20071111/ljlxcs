...
<div>
  ...
  <v-dialog
    ...
    >
      <v-card>
        <v-card-title class="headline">确定与孩子解除绑定关系?</v-card-title>

        <v-card-text>
          孩子姓名：{{unbindclilditem.uname}}({{unbindclilditem.user_id}})
        </v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn
            color="green darken-1"
            flat="flat"
            @click.stop="unbindchild(unbindclilditem)"
          >
            确定
          </v-btn>
        </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    clickUnbindChildItem (props) {
      this.dialog = true
      this.unbindclilditem = props
    },
    clickUnbindClass () {
