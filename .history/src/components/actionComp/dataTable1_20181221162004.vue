  ...
  >
    <template slot="items" slot-scope="props">
      ...
      :style="item.nowrap && {'white-space':'nowrap'}">
          ...
          <span v-else-if="item.value === 'avatarjump'" @click="">
            <v-avatar :size="25">
              <v-img :src="formatAvatar(props.item.avatar)" />
            </v-avatar>
    ...
    return {
      headersBox: {
        classManage0: [['logo_url', '基础信息', 'left', false, true], ['cnt_teacher', '老师数', 'left', true], ['cnt_parent', '家长数', 'left', true],
          ['cnt_stu', '学生数', 'left', true], ['born_year', '届(年)', 'left', false, true], ['stage', '学段-年级', 'left', true],
          ['cert', '认证状态', 'left', true], ['status', '状态', 'left', false, true], ['last_time', '时间', 'right', true]
        ],
        ...
        userManage0: [['avatar', '基础信息', 'left', false, true], ['p_id', '推广员ID', 'left', true], ['origin', '来源ID', 'left', true],
          ['mobile', '手机号', 'left', true], ['user_role', '身份', 'left', false, true], ['status', '状态', 'left', true],
          ['prof_tag', '关注公众号', 'center', false], ['country', '地区', 'left', true], ['last_time', '时间', 'right', true]
        ]
      },
      getCountry,
      getStrTime,
