<template>
    ...
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || ((image || media) && imageArr[2]) || 360" :disabled="disabledOrNo">
      ...
      <v-card>
        ...
        <img v-else-if="image" :src="[imageArr[3]]" :style="{'width':imageArr[1] + 'px','width':imageArr[1] + 'px'}">
        
        <video v-else-if="which === 'videoLook'" :src="imageArr[5]" controls="controls" :style="{'width':imageArr[1] + 'px','width':imageArr[1] + 'px'}">
          您的浏览器不支持 video 标签。
        </video>
