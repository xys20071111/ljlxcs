<template>
    ...
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || ((image || media) && imageArr[2]) || 360" :disabled="disabledOrNo">
      ...
      <v-card>
        ...
        <!-- <v-card-text v-if="which === 'unbindParent'">
          {{alertName[which]}}：{{unbindWho.uname || '暂无昵称'}}({{unbindWho.user_id}})
        </v-card-text> -->
        ...
        <v-img v-else-if="which === 'imageLook'"
          ...
          class=" lighten-2"
          :style="rotate"
          @click="imageRotate = imageRotate + 90 > 360 ? 90 : imageRotate + 90"
        >
          <v-layout
            slot="placeholder"
            fill-height
        ...
        <video v-else-if="which === 'videoLook' && dialog" loop autoplay :src="imageArr[5]" controls="controls" :width="imageArr[1]">
          您的浏览器不支持 video 标签。
        </video>

        <video v-else-if="which === 'recordLook' && dialog" loop autoplay :src="imageArr[5]"  controls="controls" :max-width="imageArr[1]">
          您的浏览器不支持 audio 标签。
        </video>
