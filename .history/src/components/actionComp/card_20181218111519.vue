    ...
    <v-card tile>
      ...
      >
        <template slot="item" slot-scope="props">
          <v-list dense>
            <v-layout row wrap>
              <v-flex v-for="(item, index) in allKey[which]" :key="index" mb-2>
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-content @click="$emit('jumpToChildClass', props.item.class_id, which)" class="align-end" v-else-if="value.eng === 'class_id' && (which==='class')">
