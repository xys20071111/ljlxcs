            ...
            <v-layout row wrap mb-2>
              ...
              >
                <v-list-tile v-for="(value, key) in item" :key="key" class="listTile">
                  ...
                  <v-list-tile-content @click="endTime - startTime < 500 && clickOndList(props.item[value[0]], value[0])" class="align-end"
                    v-else-if="value[0] === 'user_id' && (which==='child' || which==='parent' || which==='orderGods') || value[0] === 'author' || value[0] === 'owner'">
                    <span @dblclick="endTime - startTime < 500 && clickOndList()">{{ props.item[value[0]] }}</span>
                  </v-list-tile-content>
