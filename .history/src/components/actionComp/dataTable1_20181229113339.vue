<template>
  ...
  >
    <template slot="items" slot-scope="props">
      <tr @click.stop="endTime - startTime < 500 && clickListOne(props)" :style="clickWhich && clickWhich[0] && clickWhich[1] === props.index && {'background':'#4FC3F7','color':'#fff'}">
        <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index" :style="item.nowrap && {'white-space':'nowrap'}" style="word-break:break-all"
          @click="endTime - startTime < 500 && clickTdOne($event, props.item[item.value] || props.item.user_id, item.value )" @dblclick="endTime - startTime < 500 && clickTdOne($event, '', item.value)"
