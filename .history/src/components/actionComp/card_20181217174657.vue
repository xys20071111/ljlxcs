            ...
            <v-layout row wrap>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-content @click="$emit('jumpToChild', props.item.class_id)" class="align-end" v-else-if="value.eng === 'class_id' && (which==='class')">
                    <span @dblclick="$emit('jumpToChild')">{{ props.item.class_id }}</span>
                  </v-list-tile-content>
