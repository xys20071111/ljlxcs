<template>
  ...
  >
    <template slot="items" slot-scope="props">
      <tr @click.stop="clickListOne(props)" :style="drawer && clickWhich === props.index && {'background':'#4FC3F7','color':'#fff'}">
...
export default {
  components: {},
  props: ['which', 'desserts', 'loading', 'totalDesserts', 'news', 'drawer'],
  watch: {
    desserts () {
  ...
  methods: {
    ...
    clickListOne (props) {
      if (this.which === 'classManage1') {
        ...
        this.clickWhich = props.index
        this.$emit('clickList', props.item)
      }
    }
  }
}
