<template>
    ...
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || ((image || media) && imageArr[2]) || 360" :disabled="disabledOrNo">
      ...
      <v-card>
        ...
        <v-img v-else-if="image" :src="[imageArr[3]]" :width="imageArr[1]" :height="imageArr[1]" aspect-ratio="1" class=" lighten-2">
          <v-layout
            slot="placeholder"
            fill-height
            align-center
        ...
        <v-img v-else-if="which === 'imageLook'"
          ...
          :width="imageArr[1]"
          :style="rotate"
          @click="imageRotate = imageRotate + 90 > 360 ? imageRotate + 90 : 90"
        >
          <v-layout
            slot="placeholder"
            fill-height
