            ...
            <v-layout row wrap>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-avatar class="align-end" v-else-if="value.eng === 'avatar' || value.eng === 'logo_url'"><img :src="props.item.avatar?formatAvatar(props.item.avatar):formatAvatar(props.item.logo_url)"></v-list-tile-avatar>
