<template>
    ...
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || ((image || media) && imageArr[2]) || 360" :disabled="disabledOrNo">
      <slot slot="activator" name="slotButton">
        <v-img v-if="image" slot="activator" :style="{'width':imageArr[0] + 'px','width':imageArr[0] + 'px'}" 
      ...
      <v-card>
        ...
        <v-img v-else-if="image" :src="imageArr[3]" :lazy-src="noImage" aspect-ratio="1" class="lighten-2">
          <v-layout
            slot="placeholder"
            fill-height
            align-center
