            ...
            <v-layout row wrap>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-content class="align-end" v-else-if="value.eng === 'stage'">{{ props.item.stage }}-{{ props.item.grade }}-{{ props.item.seq_id }}</v-list-tile-content>
                  ...
                  <v-list-tile-content @click="$emit('jumpToChild', props.item.user_id)" class="align-end" v-else-if="value.eng === 'user_id' && (which==='child' || which==='parent')">
                    <span @dblclick="$emit('jumpToChild')">{{ props.item.user_id }}</span>
                  </v-list-tile-content>
                  <v-list-tile-content @click="$emit('jumpToClass', props.item.class_id)" class="align-end" v-else-if="value.eng === 'class_id' && (which==='class')">
                    <span @dblclick="$emit('jumpToClass')">{{ props.item.class_id }}</span>
                  </v-list-tile-content>
...
<script>
  import { formatGender, formatUserRole, formatAvatar, formatClassMode } from '@/api/dist.js'
  export default {
    data () {
      return {
        ...
        allKey: {
          ...
          onlyClass: [
            [{eng: 'class_id', chn: '班级ID:'}, {eng: 'cnt_teacher', chn: '教师人数:'}, {eng: 'cnt_stu', chn: '学生人数:'}, {eng: 'cnt_parent', chn: '家长人数:'}, {eng: 'born_year', chn: '届(年):'}],
            [{eng: 'class_mode', chn: '班级性质:'}, {eng: 'tag_open', chn: '允许老师加入:'}, {eng: 'in_time', chn: 'in_time:'}, {eng: 'last_time', chn: 'last_time:'}]
          ]
        },
        formatGender,
        formatUserRole,
