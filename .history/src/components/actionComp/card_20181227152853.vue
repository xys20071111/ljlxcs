            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key" class="listTile">
                  ...
                  <v-list-tile-content class="align-end" v-else-if="value[0] === 'recv_range'">{{ ['班级', '学校'][props.item.recv_range] }}</v-list-tile-content>
      ...
      return {
        ...
        allKey: {
          ...
          msgManage: [
            ...
            [['recv_range', '范围:'], ['uriVideo', '视频个数:'], ['uriRecord', '录音个数:'], ['faved', '是否收藏:'], ['thumbed', '是否点赞:'],
              ['in_time', '发布时间:'], ['ip', 'ip:']
            ]
          ]
        },
