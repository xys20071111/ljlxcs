    ...
    <template slot="items" slot-scope="props">
      ...
      :style="item.nowrap && {'white-space':'nowrap'}">
          ...
          <span v-else-if="item.value === 'status'" :class="['subheading',props.item.status===200?'green--text':'red--text']" style="white-space:nowrap">
            ...
            {{ which === 'userManage0' ? formatStatus(props.item.status) : formatClsStatus(props.item.status) }}
          </span>
...
import { getCountry, getStrTime, getStrFixed, formatUserRole, getStrClass, getStrMan, formatStageGrade,
  formatClsStatus, formatStatus, formatAvatar, formatEduRole } from '../../api/dist.js'
export default {
  components: {},
