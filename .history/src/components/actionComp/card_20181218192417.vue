            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-content class="align-end" v-else-if="value.eng === 'stage'">{{ formatStageGrade(props.item.stage, props.item.grade)[1] }}</v-list-tile-content>
                  <v-list-tile-content class="align-end" v-else-if="value.eng === 'grade'">{{ formatStageGrade(props.item.stage, props.item.grade)[0] }}</v-list-tile-content>
    ...
    data () {
      return {
        ...
        allKey: {
          ...
          onlyClass: [
            [{eng: 'class_id', chn: '班级ID:'}, {eng: 'cnt_teacher', chn: '教师人数:'}, {eng: 'cnt_stu', chn: '学生人数:'}, {eng: 'cnt_parent', chn: '家长人数:'}, {eng: 'born_year', chn: '届(年):'}, {eng: 'stage', chn: '学段:'}],
            [{eng: 'class_mode', chn: '班级性质:'}, {eng: 'tag_open', chn: '允许老师加入:'}, {eng: 'tag_open', chn: '允许学生加入:'}, {eng: 'in_time', chn: 'in_time:'}, {eng: 'last_time', chn: 'last_time:'}, {eng: 'grade', chn: '年级:'}]
          ]
        },
        formatGender,
        formatUserRole,
