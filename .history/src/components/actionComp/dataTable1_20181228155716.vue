<template>
  ...
  >
    <template slot="items" slot-scope="props">
      ...
      :style="item.nowrap && {'white-space':'nowrap'}" style="word-break:break-all" @click="clickTdOne($event, props.item[item.value] || props.item.user_id, item.value )">
...
export default {
  ...
  methods: {
    ...
    // 点击某一条的时候查看详情
    clickListOne (props) {
      if (['classManage0', 'userManage0', 'msgManage0', 'hwManage0'].indexOf(this.which) !== -1) {
        this.$emit('clickList', props.item, props.index)
      }
    },
    clickTdOne (e, item, itemValue) {
      if (e.stopPropagation && (itemValue === 'owner' || (this.which === 'classManage1' && itemValue === 'avatarjump'))) {
        e.stopPropagation()
      } else if (itemValue === 'owner' || (this.which === 'classManage1' && itemValue === 'avatarjump')) {
        window.event.cancelBubble = true
      }
      if (itemValue === 'owner' || (this.which === 'classManage1' && itemValue === 'avatarjump')) {
        this.$emit('jumpToMan', item)
      }
    }
  }
}
