  ...
  >
    <template slot="items" slot-scope="props">
      <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index" :style="item.nowrap && {'white-space':'nowrap'}">
        ...
        <span v-if="item.value === 'pubtime'">{{ getStrTime(props.item[item.value]) }}</span>
        <span v-if="item.value === 'amount_status'">{{['收入', '支出'][props.item[item.value] * 1 - 1]}}</span>
        <span v-if="['read_rate', 'submit_rate', 'avgreadrate', 'avgsubmitrate', 'maxreadrate', 'maxsubmitrate', 'perc'].indexOf(item.value) !== -1">
...
<script>
import { getCountry, getStrTime, getStrFixed } from '../../api/dist.js'
export default {
  components: {},
