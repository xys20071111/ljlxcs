            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-content @click="$emit('jumpToChildClass', props.item.user_id, which, value[0])" class="align-end" v-else-if="value[0] === 'user_id' && (which==='child' || which==='parent') || value[0] === 'spaceClass'">
