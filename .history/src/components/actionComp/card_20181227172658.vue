            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key" class="listTile">
                  <v-list-tile-content class="grey--text" style="white-space:nowrap;overflow:visible">{{value[1]}}</v-list-tile-content>
                  ...
                  <v-list-tile-content class="align-end" v-else-if="value[0] === 'msgImage' || value[0] === 'msgVideo' || value[0] === 'msgRecord'">
                    {{ props.item.attachment && props.item.attachment.filter(val => {
                      return {'msgImage': ['simg', 'cimg', 'vimg'], 'msgVideo': ['video', 'svideo'], 'msgRecord': ['record']}[value[0]].indexOf(val.sort) !== -1}).length
                    }}
                  </v-list-tile-content>


                  <v-list-tile-content @click="$emit('jumpToChildClass', props.item[value[0]], which, value[0])" class="align-end"
                    v-else-if="value[0] === 'user_id' && (which==='child' || which==='parent') || value[0] === 'author' || value[0] === 'owner'">
                    <span @dblclick="$emit('jumpToChildClass')">{{ props.item[value[0]] }}</span>
                  </v-list-tile-content>
                  <v-list-tile-content @click="$emit('jumpToChildClass', props.item.class_id, which, value[0])" class="align-end" v-else-if="value[0] === 'class_id' && (which==='class' || which === 'stuclass'|| which==='spaceClass')">
    ...
    data () {
      return {
        ...
        allKey: {
          ...
          hwManage: [
            [['hw_id', '作业ID:'], ['hw_type', '作业类型:'], ['owner', '作者ID:'], ['content', '内容:'], ['cnt_class', '发布班级数:'], ['cnt_read', '班级阅读数:'], ['cnt_comment', '评论数:'],
              ['title', '标题:'], ['pub_time', '发布时间:'], ['start_time', '作业开始时间:'], ['end_time', '作者截止时间:'],
              ['msgCountry', '发布地区:'], ['recv_range', '作业范围:']
            ],
            [['row_id', '数据ID:'], ['submit_type', '作业要求的类型:'], ['see_answer', '查看回复:'], ['see_stuanswer', '查看学生回复:'], ['course', '相关的科目ID:'],
              ['msgImage', '图片个数:'], ['msgVideo', '视频个数:'], ['msgRecord', '录音个数:'], ['faved', '是否收藏:'], ['thumbed', '是否点赞:'],
              ['msgStatus', '状态:'], ['to', '指定群组发作业ID:'], ['tag_comment', '开放状态'], ['tag_behf', '行为标识']
            ]
          ]
        },
        formatGender,
