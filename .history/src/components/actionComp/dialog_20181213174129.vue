        ...
        <v-card-text  v-if="which === 'setMan'">
          <v-container grid-list-md style="padding:0">
            <v-layout wrap align-center>
              ...
              <v-text-field placeholder="请输入用户昵称" v-model="setMan[0]"></v-text-field>
            </v-layout>
            <v-layout nowrap align-center>
              <v-flex style="flex:0 0 auto;font-size:16px" blue--text>用户性别：</v-flex>
              <v-radio-group v-model="setMan[1]" hide-details row style="flex: 1;margin:0">
                <v-radio
            ...
            <v-layout nowrap align-center v-if="setMan[2] !== 1">
              <v-flex style="flex:0 0 auto;font-size:16px" blue--text>用户身份：</v-flex>
              <v-radio-group v-model="setMan[2]" hide-details row style="flex: 1;margin:0">
                <v-radio
