<template>
  <v-layout style="display:inline-flex">
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || 360" :disabled="quitOrNo">
      <v-btn slot="activator" :flat="which === 'quitclass'||which === 'addMember'" :disabled="quitOrNo" :small="which === 'addMember'" :color="color">{{butCont}}</v-btn>
      <v-card>
        <v-card-title>
  ...
  export default {
    data () {
      return {
        ...
        alertCont: {'unbindchild': '确定与孩子解除绑定关系？', 'unbindParent': '确定与家长解除绑定关系？', 'freezeMan': '确定冻结该用户账号？', 'unbindClass': '确定退出该班级？', 'quitclass': '确定退出该班级？'},
