            ...
            <v-layout row wrap mb-2>
              ...
              >
                <v-list-tile v-for="(value, key) in item" :key="key" class="listTile"
                  @click.native="endTime - startTime < 400 && clickOneCard(props.item, value[0])"
                  @dblclick.native="endTime - startTime < 400 && clickOneCard('', value[0])"
                >
                  ...
                  <v-list-tile-avatar class="align-end" v-else-if="value[0] === 'avatar' || value[0] === 'logo_url'">
                    <!-- <img :src="formatAvatar(props.item.avatar || props.item.logo_url)"> -->
                    <Dialog image="true" :imageArr="[40,310,310,formatAvatar(props.item.avatar || props.item.logo_url)]"
                      :disabledOrNo="!props.item.avatar && !props.item.logo_url">
                    </Dialog>
                  </v-list-tile-avatar>
      ...
      clickOneCard (item, itemValue) {
        if (item) {
          ...
          this.timeout = window.setTimeout(() => {
            if ((itemValue === 'user_id' && (['child', 'parent', 'orderGods'].includes(this.which))) ||
              (itemValue === 'class_id' && (['class', 'stuclass', 'spaceClass'].includes(this.which))) ||
              ['author', 'owner'].includes(itemValue)
            ) {
              this.$emit('jumpToUserClass', item[itemValue], this.which, itemValue)
            }
          }, 200)
        } else {
          clearTimeout(this.timeout)
