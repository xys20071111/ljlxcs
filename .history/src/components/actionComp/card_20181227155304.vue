            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key" class="listTile">
                  ...
                  <v-list-tile-content class="align-end" v-else-if="value[0] === 'cnt_read'">{{ props.item.summary&&props.item.summary.cnt_read }}</v-list-tile-content>
    ...
    data () {
      return {
        ...
        allKey: {
          ...
          msgManage: [
            [['m_id', 'm_id:'], ['owner', '作者ID:'], ['content', '内容:'], ['cnt_class', '发布班级数:'], ['cnt_read', '班级阅读数:'], ['cnt_comment', '评论数:'],
              ['title', '标题:'], ['pub_time', '发布时间:'], ['msgCountry', '地区:']
            ],
            [['recv_range', '范围:'], ['row_id', '数据ID:'], ['msgImage', '图片个数:'], ['msgVideo', '视频个数:'], ['msgRecord', '录音个数:'], ['faved', '是否收藏:'], ['thumbed', '是否点赞:'],
              ['msgStatus', '状态:'], ['to', '指定群组发作业ID:']
            ]
          ]
        },
        formatGender,
