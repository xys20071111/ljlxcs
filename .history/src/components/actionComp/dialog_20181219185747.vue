<template>
  <v-layout style="display:inline-flex">
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || (image && imageArr[2]) || 360" :disabled="quitOrNo">
      <v-btn v-if="!image" slot="activator" :flat="which === 'quitclass'||which === 'addMember'" :disabled="quitOrNo" :small="which === 'quitclass'" :color="color">{{butCont}}</v-btn>
      ...
      <v-card>
        ...
        <img v-if="image" :src="[imageArr[3]]" :style="{'width':imageArr[1] + 'px','width':imageArr[1] + 'px'}">
