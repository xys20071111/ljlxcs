            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-content class="align-end" v-else-if="value[0] === 'class_name'">{{ props.item.class_name && (props.item.class_name.slice(-1) === '班' ? props.item.class_name : props.item.class_name + '班')}}</v-list-tile-content>
                  ...
                  <v-list-tile-content class="align-end" v-else-if="value[0] === 'status'">{{ formatStatus(props.item.status) }}</v-list-tile-content>
      ...
      return {
        ...
        allKey: {
          userinfo: [
            ...
            [['gender', '性别:'], ['user_role', '身份:'], ['p_id', '推广员ID:'], ['origin', '来源ID:'], ['country', '地区:']],
            [['in_time', '创建时间:'], ['last_time', '最近更改时间:'], ['intro', '个人签名:']]
          ],
          child: [
          ...
          onlyClass: [
            [['class_id', '班级ID:'], ['def_name', '系统班级名称:'], ['class_name', '自定义班级名称:'], ['cnt_teacher', '教师人数:'], ['cnt_stu', '学生人数:'], ['cnt_parent', '家长人数:'], ['born_year', '届(年):'], ['stage', '学段:'], ['grade', '年级:'], ['cert', '认证状态:']],
            [['class_mode', '班级性质:'], ['status', '状态:'], ['tag_open', '允许老师加入:'], ['tag_open', '允许学生加入:'], ['graduated', '是否毕业:'], ['school_id', '学校ID:'], ['seq_id', '班级号:'], ['last_upt_user', '最近更新人:'], ['in_time', '创建时间:'], ['last_time', '最近更改时间:']]
          ],
          orderGods: [
            [['amount', '变更金额:'], ['amount_status', '收支情况:'], ['class_name', '自定义班级名称:'], ['cnt_teacher', '教师人数:'], ['cnt_stu', '学生人数:'], ['cnt_parent', '家长人数:'], ['born_year', '届(年):'], ['stage', '学段:'], ['grade', '年级:'], ['cert', '认证状态:']],
            [['class_mode', '班级性质:'], ['status', '状态:'], ['tag_open', '允许老师加入:'], ['tag_open', '允许学生加入:'], ['graduated', '是否毕业:'], ['school_id', '学校ID:'], ['seq_id', '班级号:'], ['last_upt_user', '最近更新人:'], ['in_time', '创建时间:'], ['last_time', '最近更改时间:']]
          ]
        },
        formatGender,
