<template>
    ...
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || ((image || media) && imageArr[2]) || 360" :disabled="disabledOrNo">
      ...
      <v-card>
        ...
        <v-flex  v-else-if="which === 'imageLook'" @click="clickImage" ref="clickImage">
          <v-img
            :src="imageArr[3]"
            :lazy-src="imageArr[1]"
            aspect-ratio="1"
...
<script>
  export default {
    ...
    methods: {
      ...
      clickImage (e) {
        console.log(this.$refs.clickImage.$el.clientHeight, e)
        this.imageRotate = this.imageRotate + 90 > 360 ? 90 : this.imageRotate + 90
      }
