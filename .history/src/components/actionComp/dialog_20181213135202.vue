<template>
  <v-layout style="display:inline-flex">
    <v-dialog v-model="dialog" max-width="600px">
      ...
      <v-card>
        <v-card-title>
          <span class="headline">设置用户信息</span>
        </v-card-title>
        <v-card-text>
          <v-container grid-list-md style="padding:0">
            <v-layout wrap align-center>
                <v-flex grow style="font-size:16px" blue--text>用户昵称：</v-flex>
                <v-text-field :placeholder="123"></v-text-field>
            </v-layout>
            <v-layout wrap align-center>
              <v-radio-group v-model="state" hide-details row style="margin: 10px 10px 10px 0px;flex: 0 0 auto;">
            ...
            <v-layout nowrap align-center>
              ...
              <v-radio-group v-model="state" hide-details row style="margin: 10px 10px 10px 0px;flex: 1;">
                <v-flex style="display:flex;flex-wrap:wrap">
                  <v-radio
                    v-for="(value, key) in ['学生', '家长', '老师']"
                    :key="value"
                    :label="value"
                    :value="key + 1"
                    color="#2196f3"
