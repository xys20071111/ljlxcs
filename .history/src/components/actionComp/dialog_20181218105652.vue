<template>
  <v-layout style="display:inline-flex">
    <v-dialog v-model="dialog" :max-width="(alert && 290) || (which === 'addChild' && 450) || 360" :disabled="quitOrNo">
      <v-btn slot="activator" :flat="which === 'quitclass'" disabled :small="which === 'quitclass'" :color="color">{{butCont}}</v-btn>
      <v-card>
        <v-card-title>
