  ...
  >
    <template slot="items" slot-scope="props">
      ...
      :style="item.nowrap && {'white-space':'nowrap'}" style="word-break:break-all">
          ...
          <span v-else-if="item.value === 'cnt_read'">{{ props.item.summary&&props.item.summary.cnt_read }}</span>
  ...
  data () {
    return {
      headersBox: {
        ...
        msgManage0: [['m_id', '通知ID', 'left', false, true], ['title', '标题', 'left', true], ['owner', '作者ID', 'left', true, true],
          ['content', '内容', 'left', false], ['cnt_class', '发布班级数', 'left', true], ['cnt_read', '班级阅读数', 'left', true],
          ['cnt_comment', '评论数', 'center', true], ['pub_time', '发布时间', 'right', false]
        ]
      },
      getCountry,
      getStrClass,
