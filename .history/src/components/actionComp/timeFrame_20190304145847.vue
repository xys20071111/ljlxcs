<template>
  <div class="wrapper">
    ...
    >
      ...
      >
        <v-spacer></v-spacer>
        <v-btn flat color="primary" v-if="!month" @click="oneDayAgo(7)">一周前</v-btn>
        <v-btn flat color="primary" v-if="!month" @click="oneDayAgo(30)">一月前</v-btn>
    ...
    >
      ...
      >
        <v-spacer></v-spacer>
        <v-btn small flat color="primary" v-if="!startHide && !month" @click="oneDayAgo(7)">一周前</v-btn>
        <v-btn small flat color="primary" v-if="!startHide && !month" @click="oneDayAgo(30)">一月前</v-btn>
...
export default {
  ...
  methods: {
    noAllow (e) {
      ...
      let lastDate = new Date(lastTime[0] * 1, lastTime[1] * 1 - 1, lastTime[2] * 1 || 0).getTime()
      let nowDate = new Date(thisTime[0] * 1, thisTime[1] * 1 - 1, thisTime[2] * 1 || 0).getTime()
  ...
  created () {
    ...
    this.date = new Date(new Date().getTime() - 86400000 * startDay).toISOString().substr(0, this.month ? 7 : 10)
  },
  mounted () {
    this.$emit('change', [this.date, this.dates])
  }
