            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key" class="listTile">
                  ...
                  <v-list-tile-content @click="$emit('jumpToChildClass', props.item[value[0]], which, value[0])" class="align-end" v-else-if="value[0] === 'user_id' && (which==='child' || which==='parent') || value[0] === 'author'">
