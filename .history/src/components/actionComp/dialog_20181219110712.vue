<template>
  <v-layout style="display:inline-flex">
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || 360" :disabled="quitOrNo">
      <v-btn slot="activator" :flat="which === 'quitclass'||which === 'addMember'" :disabled="quitOrNo" :small="which === 'quitclass'" :color="color">{{butCont}}</v-btn>
      <v-card>
        <v-card-title>
        ...
        <v-card-text  v-if="which === 'addMember'">
          <v-container grid-list-md style="padding:0">
            <v-layout wrap align-center>
        ...
        <v-card-text v-if="which === 'unbindchild' || which === 'unbindParent'">
        ...
        <v-card-text v-if="which === 'unbindClass' || which === 'cancelClass'">
  ...
  export default {
    data () {
      return {
        ...
        alertCont: {'unbindchild': '确定与孩子解除绑定关系？', 'unbindParent': '确定与家长解除绑定关系？', 'freezeMan': '确定冻结该用户账号？', 'unbindClass': '确定退出该班级？', 'quitclass': '确定退出该班级？', 'addMember': '添加成员'},
        alertName: {'unbindchild': '孩子名称', 'unbindParent': '家长名称', 'freezeMan': '用户名称', 'unbindClass': '要退出的班级', 'quitclass': '成员姓名'}
      }
    },
    components: {
