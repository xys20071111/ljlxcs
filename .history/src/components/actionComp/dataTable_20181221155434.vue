<template>
  <v-data-table
    ...
    class="elevation-1"
  >
    <template slot="items" slot-scope="{propItem: item}">
      <tr @click="['allIntegral1', 'reconIntergral2'].indexOf(which) !== -1 ? clickDetail(propItem):''">
        <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index"
        :style="item.nowrap && {'white-space':'nowrap'}">
          <span v-if="item.value === 'city'">{{ getCountry(propItem.country, propItem.province, propItem.city) }}</span>
          <span v-else-if="item.value === 'pubtime'">{{ getStrTime(propItem.pubtime) }}</span>
          <span v-else-if="item.value === 'class_name'">{{ getStrClass(propItem.class_name)[0] }}</span>
          <span v-else-if="item.value === 'amount_status'">{{['收入', '支出'][propItem.amount_status * 1 - 1]}}</span>
          <span v-else-if="item.value === 'currecy_id'">{{['积分', '人民币'][propItem.currecy_id * 1 - 1]}}</span>
          <span v-else-if="fixedBox.indexOf(item.value) !== -1">
            {{getStrFixed(propItem[item.value])}}
          </span>
