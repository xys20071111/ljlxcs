<template>
  <div class="wrapper">
    ...
    <span class="timeOver" v-if="!endHide" style="margin-right: 2px">至</span>
    <v-menu
      :close-on-content-click="false"
      v-model="menus"
      :nudge-right="40"
    ...
    >
      ...
      <v-date-picker 
        ...
        @input="timeinput"
        locale="zh-cn"
        v-bind="!endHide ? {'allowed-dates':noAllows}:{}"
        :type="month ? 'month' : 'date'"
      >
        <v-spacer></v-spacer>
        <v-btn small flat color="primary" v-if="!endHide" @click="oneDayAgo(7)">一周前</v-btn>
        <v-btn small flat color="primary" v-if="!endHide" @click="oneDayAgo(30)">一月前</v-btn>
...
export default {
  components: {},
  props: ['start', 'endHide', 'month'],
  data () {
    return {
      menu: false,
      menus: false,
      dates: new Date().toISOString().substr(0, this.month ? 7 : 10),
      // vDates: new Date().toISOString().substr(0, this.month ? 7 : 10).replace('-', '年').replace('-', '月') + (this.month ? '月' : '日'),
      date: '',
      vDate: ''
    }
  },
  computed: {
    vDates () {
      return this.vDates.replace('-', '年').replace('-', '月') + (this.month ? '月' : '日')
    }
  },
  watch: {
    date (e) {
      console.log(e)
      this.vDate = e.replace('-', '年').replace('-', '月') + '日'
    }
    // dates (e) {
    //   console.log(e)
    //   this.vDates = e.replace('-', '年').replace('-', '月') + (this.month ? '月' : '日')
    // }
  },
  methods: {
    noAllow (e) {
  ...
  created () {
    ...
    this.vDate = new Date(new Date().getTime() - 86400000 * startDay).toISOString().substr(0, 10).replace('-', '年').replace('-', '月') + '日'
  },
  mounted () {
    this.$emit('change', [this.date, this.dates])
  }
