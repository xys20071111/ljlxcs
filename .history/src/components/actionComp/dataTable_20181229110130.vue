    ...
    <template slot="items" slot-scope="props">
      <tr @click="clickListOne(props)" :style="clickWhich && clickWhich[0] && clickWhich[1] === props.index && {'background':'#4FC3F7','color':'#fff'}">
        <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index"
          ...
          @dblclick="clickTdOne($event, '', item.value)" @touchstart="startTime = $event.timeStamp" @touchend="startEnd = $event.timeStamp" 
          @mousedown="startTime = $event.timeStamp" @mouseup="startEnd = $event.timeStamp"
        >
