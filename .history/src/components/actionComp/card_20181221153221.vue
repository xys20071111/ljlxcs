            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-content class="align-end" v-else-if="value[0] === 'clsstatus'">{{ formatClsStatus(props.item.status) }}</v-list-tile-content>
...
<script>
  ...
  formatStageGrade, formatStatus, formatClsStatus, getStrClass } from '@/api/dist.js'
  export default {
    data () {
      return {
