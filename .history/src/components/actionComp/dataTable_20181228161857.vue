<template>
  ...
  >
    <template slot="items" slot-scope="props">
      <tr @click="clickListOne(props)" :style="clickWhich && clickWhich[0] && clickWhich[1] === props.index && {'background':'#4FC3F7','color':'#fff'}">
        <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index"
        :style="item.nowrap && {'white-space':'nowrap'}" @click="clickTdOne($event, props.item[item.value], item.value)">
...
export default {
  ...
  methods: {
    ...
    clickListOne (props) {
      if (['allIntegral1', 'reconIntergral2', 'classSpace0'].indexOf(this.which) !== -1) {
        this.$emit('clickList', props.item, props.index)
      }
    },
    clickTdOne (e, item, itemValue) {
      if (e.stopPropagation && (itemValue === 'owner' || itemValue === 'author' || (itemValue === 'class_id' && (this.which === 'classSpace0' || this.which === 'spaceStat')))) {
        e.stopPropagation()
      } else if (itemValue === 'owner' || itemValue === 'author' || (itemValue === 'class_id' && (this.which === 'classSpace0' || this.which === 'spaceStat'))) {
        window.event.cancelBubble = true
      }
      if (itemValue === 'author' || itemValue === 'owner' || (itemValue === 'class_id' && (this.which === 'classSpace0' || this.which === 'spaceStat'))) {
