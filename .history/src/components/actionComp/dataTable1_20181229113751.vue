<template>
  ...
  >
    <template slot="items" slot-scope="props">
      <tr @click.stop="endTime - startTime < 500 && clickListOne(props)"
      ...
      >
        <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index"
          ...
          @click="endTime - startTime < 500 && clickTdOne($event, props.item[item.value] || props.item.user_id, item.value )"
