<template>
    ...
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || ((image || media) && imageArr[2]) || 360" :disabled="disabledOrNo">
      <slot slot="activator" name="slotButton">
        <img v-if="image" slot="activator" :style="{'width':imageArr[0] + 'px','width':imageArr[0] + 'px'}" :src="imageArr[3]">
        ...
        <v-btn v-else slot="activator" :flat="which === 'quitclass'||which === 'addMember'" :disabled="disabledOrNo" :small="which === 'quitclass'" :color="color">{{butCont}}</v-btn>
      </slot>
      <v-card>
        ...
        <v-img v-else-if="image" :src="imageArr[3]" aspect-ratio="1" class=" lighten-2">
          <v-layout
            slot="placeholder"
            fill-height
            align-center
        ...
        <v-img v-else-if="which === 'imageLook'"
          :src="imageArr[3]"
          :lazy-src="imageArr[1]"
          aspect-ratio="1"
          class=" lighten-2"
        ...
        <video v-else-if="which === 'videoLook' && dialog" loop autoplay :src="imageArr[4]" controls="controls" width="100%">
          您的浏览器不支持 video 标签。
        </video>

        <video v-else-if="which === 'recordLook' && dialog" loop autoplay :src="imageArr[4]"  controls="controls" width="100%">
          您的浏览器不支持 audio 标签。
        </video>
...
<script>
  export default {
    ...
    methods: {
      ...
      clickImage (e) {
        // console.log(this.imageRotate, e.offsetX, e.offsetY)
        let imageWidth = this.$refs.clickImage.$el.clientWidth
        let distanceLeft = this.imageRotate > 90 ? (imageWidth - e.offsetX) : e.offsetX
        console.log(distanceLeft, this.imageRotate)
        let wiseOrNo = distanceLeft * 2 > imageWidth ? 1 : -1
        // console.log(wiseOrNo)
        this.imageRotate = this.imageRotate + 90 * wiseOrNo > 350 ? 0 : this.imageRotate + 90 * wiseOrNo < 0 ? 270 : this.imageRotate + 90 * wiseOrNo
      }
