            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key" class="listTile" 
                  ...
                  @mousedown="alert(0)" @mouseup="endTime = $event.timeStamp"
                >
                  ...
                  <v-list-tile-content @click="clickOndList(props.item[value[0]], value[0])" class="align-end"
                    v-else-if="value[0] === 'user_id' && (which==='child' || which==='parent' || which==='orderGods') || value[0] === 'author' || value[0] === 'owner'">
                    <span @dblclick="clickOndList()">{{ props.item[value[0]] }}</span>
                  </v-list-tile-content>
                  <v-list-tile-content @click="clickOndList(props.item[value[0]], value[0])" class="align-end"
                    v-else-if="value[0] === 'class_id' && (which==='class' || which === 'stuclass'|| which==='spaceClass')"
                  >
                    <span @dblclick="clickOndList()">{{ props.item.class_id }}</span>
                  </v-list-tile-content>
                  ...
                  <v-list-tile-content @mousedown="alert(0)" class="align-end" style="word-break:break-all" v-else>{{ props.item[value[0]] }}</v-list-tile-content>
                </v-list-tile>
              </v-flex>
...
<script>
  ...
  export default {
    ...
    props: ['which', 'data'],
    methods: {
      clickOndList (value, valueText) {
        console.log(this.endTime, this.startTime)
        if (this.endTime - this.startTime > 500) {
          return false
        }
        valueText && this.$emit('jumpToUserClass', value, this.which, valueText)
        !valueText && this.$emit('jumpToUserClass')
      }
    }
  }
</script>
