<template>
  ...
  >
    <template slot="items" slot-scope="props">
      <tr @click.stop="clickListOne(props)" :style="clickWhich && clickWhich[0] && clickWhich[1] === props.index && {'background':'#4FC3F7','color':'#fff'}">
        <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index" :style="item.nowrap && {'white-space':'nowrap'}" style="word-break:break-all"
          @click="clickTdOne($event, props.item[item.value] || props.item.user_id, item.value )" @dblclick="clickTdOne($event, '', item.value)"
          @touchstart="touchTdOne($event,'start')" @touchend="touchTdOne($event,'end')" @mousedown="touchTdOne($event,'start')" @mouseup="touchTdOne($event,'end')"
        >
          ...
          <span v-else-if="item.value === 'content'" style="max-width:300px;display:inline-block">
            {{ props.item.content }}
...
export default {
  ...
  methods: {
    ...
    clickListOne (props) {
      ...
      if (['classManage0', 'userManage0', 'msgManage0', 'hwManage0'].includes(this.which)) {
    ...
    clickTdOne (e, item, itemValue) {
      ...
      if (!(itemValue === 'owner' || itemValue === 'avatarjump')) {
        return false
      }
    ...
    touchTdOne (e, startEnd) {
      startEnd === 'start' ? (this.startTime = e.timeStamp) : (this.endTime = e.timeStamp)
    }
  }
}
</script>
