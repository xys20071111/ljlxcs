<template>
  ...
  >
    <template slot="items" slot-scope="props">
      <tr @click="endTime - startTime < 400 && clickListOne(props)"
      ...
      >
        <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index"
          ...
          @click="endTime - startTime < 400 && clickTdOne($event, props.item[item.value], item.value)"
        ...
        >
          <span v-if="item.value === 'city'" style="min-width:70px;display:inline-block">
          ...
          <span v-else-if="item.value === 'cont'" style="max-width:300px;min-width:120px;display:inline-block">
            {{ props.item.cont}}
            <span v-if="props.item.uri.length>0"><v-icon color="blue lighten-2">attach_file</v-icon>({{props.item.uri.length}})</span>
          </span>
          <span v-else-if="item.value === 'title'" style="min-width:100px;display:inline-block">{{ props.item.title }}</span>
          <span v-else-if="item.value === 'in_time'" style="min-width:80px;display:inline-block">{{ props.item.in_time }}</span>
          <span v-else-if="item.value === 'remark'" style="min-width:100px;display:inline-block">{{ props.item.remark }}</span>
          <span v-else-if="item.value === 'ip'" style="min-width:60px;display:inline-block">{{ props.item.ip }}</span>
...
export default {
    ...
    return {
      headersBox: {
        ...
        classSpace0: [['obj_id', '动态ID', 'left', true, true], ['author', '发布者ID', 'left', true, true], ['class_id', '班级ID', 'left', true, true],
          ['cnt_comment', '评论数', 'left', true], ['cnt_good', '点赞数', 'left', true],
          ['faved', '是否收藏', 'left', false], ['thumbed', '是否点赞', 'left', false], ['cont', '内容', 'center', false], ['in_time', '发布时间', 'left', true],
          ['ip', 'IP', 'right', false]
        ],
        rankPunch: [['pubtime', '发布时间', 'left', true, true], ['hwcounts', '作业数', 'center', true], ['avgreadrate', '平均阅读率', 'center', true],
          ['maxreadrate', '最高阅读率', 'center', true], ['avgsubmitrate', '平均提交率', 'center', true], ['maxsubmitrate', '最高提交率', 'center', true]
        ]
      },
  ...
  methods: {
    ...
    clickListOne (props) {
      if (['allIntegral1', 'reconIntergral2', 'classSpace0', 'rankPunch'].includes(this.which)) {
    ...
    clickTdOne (e, item, itemValue) {
      if (item) {
        ...
        if (['owner', 'author'].includes(itemValue) || (itemValue === 'class_id' && (['classSpace0', 'spaceStat'].includes(this.which))) ||
