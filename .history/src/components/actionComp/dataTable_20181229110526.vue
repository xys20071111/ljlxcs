<template>
  ...
  >
    <template slot="items" slot-scope="props">
      <tr @click="startTime - endTime < 500 && clickListOne(props)" :style="clickWhich && clickWhich[0] && clickWhich[1] === props.index && {'background':'#4FC3F7','color':'#fff'}">
        <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index" :style="item.nowrap && {'white-space':'nowrap'}"
          @click="startTime - endTime < 500 && clickTdOne($event, props.item[item.value], item.value)" 
          @dblclick="startTime - endTime < 500 && clickTdOne($event, '', item.value)" @touchstart="startTime = $event.timeStamp" @touchend="endTime = $event.timeStamp" 
