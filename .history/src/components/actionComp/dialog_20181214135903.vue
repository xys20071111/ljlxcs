  ...
  export default {
    data () {
      return {
        ...
        alertCont: {'unbindchild': '确定与孩子解除绑定关系？', 'freezeMan': '确定冻结该用户账号？', 'unbindClass': '确定退出该班级？'},
        alertName: {'unbindchild': '孩子名称', 'freezeMan': '用户名称', 'unbindClass': '要退出的班级'}
      }
    },
    components: {
    ...
    methods: {
      setWhich () {
        this.dialog = false
        this.which === 'setMan' && this.$emit('setMan', this.modelMan)
        this.which === 'addChild' && this.$emit('addChild', this.childCont)
        this.which === 'unbindchild' && this.$emit('unbindchild', this.unbindWho)
        this.which === 'freezeMan' && this.$emit('freezeMan', this.unbindWho)
        this.which === 'unbindClass' && this.$emit('unbindClass', this.unbindWho)
