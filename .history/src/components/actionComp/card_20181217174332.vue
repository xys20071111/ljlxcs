            ...
            <v-layout row wrap>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-content @click="$emit('jumpToChild', props.item.user_id)" class="align-end" v-else-if="(value.eng === 'user_id' || value.eng === 'class_id') && (which==='child' || which==='class' || which==='parent')">
