<template>
  <v-layout style="display:inline-flex">
    <v-dialog v-model="dialog" :max-width="(alert && 290) || ({'addChild': 450, 'addParent': 470}[which]) || (image && imageArr[2]) || 360" :disabled="disabledOrNo">
      <slot slot="activator" name="slotButton">
        <v-btn v-if="!image" slot="activator" :flat="which === 'quitclass'||which === 'addMember'" :disabled="disabledOrNo" :small="which === 'quitclass'" :color="color">{{butCont}}</v-btn>
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn v-if="!image" color="blue darken-1" flat @click="setWhich">确定</v-btn>
        </v-card-actions>
      </v-card>
