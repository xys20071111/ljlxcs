            ...
            <v-layout row wrap>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key">
                  ...
                  <v-list-tile-avatar class="align-end" v-else-if="value.eng === 'avatar' || value.eng === 'logo_url'"><img :src="props.item.avatar || props.item.logo_url"></v-list-tile-avatar>
...
<script>
  ...
  export default {
    data () {
      return {
        allTitle: {userinfo: '用户信息', child: '孩子信息', class: '班级信息', parent: '家长信息'},
        allTitleErr: {userinfo: '尚无用户信息', child: '用户尚无孩子', class: '用户尚未创建班级', parent: '用户尚没有家长'},
        allKey: {
          userinfo: [
          ...
          parent: [
            [{eng: 'avatar', chn: '头像:'}, {eng: 'user_id', chn: '用户ID:'}, {eng: 'uname', chn: '昵称:'}, {eng: 'realname', chn: '真实姓名:'}, {eng: 'mobile', chn: '手机号:'}, {eng: 'gender', chn: '性别:'}],
            [{eng: 'country', chn: '地区:'}, {eng: 'p_id', chn: '推广员ID:'}, {eng: 'in_time', chn: 'in_time:'}, {eng: 'last_time', chn: 'last_time:'}]
          ]
        },
        formatGender,
        formatUserRole
