            ...
            <v-layout row wrap mb-2>
              <v-flex v-for="(item, index) in allKey[which]" :key="index">
                <v-list-tile v-for="(value, key) in item" :key="key" class="listTile">
                  ...
                  <v-list-tile-content @click="$emit('jumpToUserClass', props.item[value[0]], which, value[0])" class="align-end"
                    v-else-if="value[0] === 'user_id' && (which==='child' || which==='parent' || which==='orderGods') || value[0] === 'author' || value[0] === 'owner'">
                    <span @dblclick="$emit('jumpToUserClass')">{{ props.item[value[0]] }}</span>
                  </v-list-tile-content>
                  <v-list-tile-content @click="$emit('jumpToUserClass', props.item.class_id, which, value[0])" class="align-end"
                    v-else-if="value[0] === 'class_id' && (which==='class' || which === 'stuclass'|| which==='spaceClass')"
                  >
                    <span @dblclick="$emit('jumpToUserClass')">{{ props.item.class_id }}</span>
                  </v-list-tile-content>
