<template>
  ...
  >
    <template slot="items" slot-scope="props">
      <tr @click="clickDetail(props)" :style="clickWhich && clickWhich[0] && clickWhich[1] === props.index && {'background':'#4FC3F7','color':'#fff'}">
        <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index"
        :style="item.nowrap && {'white-space':'nowrap'}" @click="clickTdDetail($event, props.item[item.value], item.value)">
...
export default {
  ...
  methods: {
    ...
    clickTdDetail (e, item, itemValue) {
      if (e.stopPropagation && (itemValue === 'owner' || itemValue === 'author' || (itemValue === 'class_id' && this.which === 'classSpace0'))) {
        e.stopPropagation()
      } else if (itemValue === 'owner' || itemValue === 'author' || (itemValue === 'class_id' && this.which === 'classSpace0')) {
        window.event.cancelBubble = true
      }
      if (itemValue === 'owner') {
        this.$emit('clickjump', item)
      } else if (itemValue === 'author' || (itemValue === 'class_id' && this.which === 'classSpace0')) {
