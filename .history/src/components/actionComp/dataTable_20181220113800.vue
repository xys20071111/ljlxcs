<template>
  ...
  >
    <template slot="items" slot-scope="props">
      <td :class="'text-xs-' + (item.textclass || item.align)" v-for="(item, index) in headers" :key="index" :style="{'white-space':'nowrap'}">
  ...
  data () {
    return {
      headersBox: {
        ...
        allIntegral1: [['user_id', '用户ID', 'left', true], ['amount', '变更金额', 'center', true], ['amount_status', '收入/支出', 'center', true],
        ...
        reconIntergral1: [['pubtime', '日期', 'left', true], ['f_humans', '发放人数', 'center', true], ['f_times', '发放人次', 'center', true], ['f_amounts', '发放金额', 'center', true],
          ['x_humans', '消费人数', 'center', true], ['x_times', '消费人次', 'center', true], ['x_amounts', '消费金额', 'right', true]
        ],
        reconIntergral2: [['user_id', '用户ID', 'left', true], ['amount', '变更金额', 'center', true], ['amount_status', '收入/支出', 'center', true], ['app_id', 'appID', 'center', true], ['atom', '订单号', 'center', true],
  ...
  computed: {
    headers () {
      ...
      for (let items of this.headersBox[this.which]) {
        ...
        headersTitleOne.nowrap = items[5]
