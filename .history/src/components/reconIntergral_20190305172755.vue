<template>
   <div>
    ...
    <chart :option="option" id="chart" v-show="buttonIndex!== 0"></chart>
    <dataTable :which="'reconIntergral' + buttonIndex" :desserts="desserts" :loading="loading" v-if="buttonIndex !== 2"></dataTable>

    <dataTable1 which="reconIntergral2" :desserts="desserts" :loading="loading" :news="news" :clickWhich="clickWhich"
      :setNew.sync="news" @clickList="clickListTableItem" :totalDesserts="totalDesserts" @clickjump="jumpToUserClass"
        @getclasslist="getNewList" :pagination.sync="pagination" v-if="buttonIndex === 2">
    </dataTable1>
...
export default {
  components: {timeFrame, chart, dataTable, dataTable1, textField, Card},
  data () {
    return {
      desserts: [],
      saveData: [],
  ...
  methods: {
    ...
    getNewList (newPage) {
      ...
      if (this.buttonIndex === 2) {
        ...
        let page = !newPage ? 1 : this.pagination.page
        let size = (rowsPerPage === 1 || rowsPerPage === 0 || !newPage) ? 20 : rowsPerPage === -1 ? 100 : rowsPerPage
        axios('/asset/log?currecyID=' + this.state + '&appID=' + this.itemsCode[this.states] + detailMonth + '&page=' + page + '&size=' + size, '', {}, cb => {
          ...
          let newAllInteg = (cb.v1 || []).reverse().map((value, index) => {
          ...
          let desserts = handleStatis(cb.v1 || [])
          this.totalDesserts = cb.v2
