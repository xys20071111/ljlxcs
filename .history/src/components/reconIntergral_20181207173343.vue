<template>
   <div>
     <v-layout class="layout" style="flex-wrap: wrap;align-items:center">
      ...
      <v-layout style="margin: 10px 10px 10px 20px;flex: 0 1 auto;">
        ...
        <v-radio-group v-model="states" hide-details row style="margin: 10px 0px;padding:0;flex: 0 1 auto;">
          <v-radio
            v-for="(value, key) in ['全部', '系统调整', '积分抽奖', '系统奖励']"
            :key="value"
            :label="value"
            :value="key"
            color="#2196f3"
    ...
    <dataTable :headers="headers" :desserts="desserts" :loading="loading" v-if="buttonIndex !== 2"></dataTable>
    <dataTable :headers="headers" :desserts="desserts" :loading="loading" :news="news" :setNew.sync="news" @getNew="getNew" v-if="buttonIndex === 2"></dataTable>
    <v-img src="https://picsum.photos/500/300?image=20" lazy-src="https://picsum.photos/500/300?image=20"></v-img>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      headersBox: [],
      headersTitle: [
        {'f_humans': '发放人数', 'f_times': '发放人次', 'f_amounts': '发放金额', 'x_humans': '消费人数', 'x_times': '消费人次', 'x_amounts': '消费金额', 'y_humans': '人数（余额）', 'y_amounts': '金额（余额）'},
        {'pubtime': '日期', 'f_humans': '发放人数', 'f_times': '发放人次', 'f_amounts': '发放金额', 'x_humans': '消费人数', 'x_times': '消费人次', 'x_amounts': '消费金额'},
        {
          'user_id': '用户ID',
          'amount': '变更金额',
          'amount_status': '收入/支出',
  ...
  mounted () {
    for (let [index, items] of this.headersTitle.entries()) {
      let headersTitle = []
      for (let [key, value] of Object.entries(items)) {
        ...
        headersTitleOne.sortable = index !== 2
