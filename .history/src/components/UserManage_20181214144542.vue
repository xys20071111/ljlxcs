        ...
        <v-container fluid grid-list-md>
          <v-layout row wrap>
            ...
            <v-flex sm12 d-flex >
              <v-card tile flat>
                <v-list dense>
                  <Dialog color="info" butCont="编辑用户信息" :setMan="setMan" which="setMan" @setMan="unbindClass"></Dialog>
                  <Dialog color="info" butCont="冻结用户" alert="true" :unbindWho="setMan" which="freezeMan" @freezeMan="unbindClass"></Dialog>
                  ...
                  <Dialog v-if="(!items.ex || !items.ex.childlist || items.ex.childlist.length < 3) && items.user_role > 1" color="info"
                    :setMan="setMan" butCont="添加孩子" which="addChild" @addChild="unbindClass">
                  </Dialog>
                  <Dialog color="info" v-if="items.user_role !== 2" butCont="添加班级" which="addClass" @addClass="unbindClass"></Dialog>
                </v-list>
              </v-card>
            </v-flex>
              ...
              <v-card tile>
                ...
                <v-list dense>
                  <v-layout row wrap>
                    ...
                    <v-flex md12>
                      <v-list-tile>
                        <Dialog color="info" butCont="解绑孩子" alert="true" :unbindWho="props.item" which="unbindchild" @unbindchild="unbindchild"></Dialog>
                      </v-list-tile>
                    </v-flex>
                  </v-layout>
                </v-list>
                      ...
                      <v-layout row wrap>
                        ...
                        <v-flex md6>
                          <v-list-tile>
                            ...
                            <Dialog color="info" butCont="退出班级" alert="true" :unbindWho="props.item" which="unbindClass" @unbindClass="unbindClass"></Dialog>
                          </v-list-tile>
                        </v-flex>
                      </v-layout>
                    </v-list>
...
export default {
  ...
  methods: {
    ...
    // 退出班级 加入班级 设置用户信息
    unbindClass (props, whichEle) {
      console.log(props, whichEle)
      let whichMethod = {addClass: 'joinclass', unbindClass: 'quitclass', setMan: 'updateuser', freezeMan: 'frozenuser', addChild: 'bindchild'}
      let whichData = {}
      ...
      whichEle === 'addChild' && (whichData = {uid: this.setMan[3], childid: props[0] * 1, ParentType: props[1] * 1})
    ...
    freezeMan (props) {
      ...
      //   .then(res => {
      //     if (res.data.code === 0) {
      //       this.$emit('update:setVsnackbar', [true, '修改成功', 'success'])
      //       return false
      //     }
      //     this.$emit('update:setVsnackbar', [true, res.data.msg, 'error'])
      //   })
    },
    // 解绑孩子
    unbindchild (props) {
      let params = {
        ...
        param: {
          method: 'unbindchild',
          data: JSON.stringify({'uid': this.items.user_id, 'childid': props.user_id})
        }
      }
      api
        .post(params)
      ...
      api
        ...
        .then(res => {
          if (res.data.code === 0) {
            this.$emit('update:setVsnackbar', [true, '修改成功', 'success'])
            this.setMan = manArr
            this.items = Object.assign({}, this.items, {'uname': manArr[0], 'gender': manArr[1], 'user_role': manArr[2]})
            return false
          }
          this.$emit('update:setVsnackbar', [true, res.data.msg, 'error'])
        })
    ...
    // 添加孩子
    addChild (childCont) {
      if (!childCont[0] || isNaN(childCont[0] * 1) || childCont[0] * 1 === this.setMan[3]) {
        this.$emit('update:setVsnackbar', [true, '请输入正确的孩子ID', 'error'])
        return false
      }
      let params = {
        ...
        param: {
          ...
          data: JSON.stringify({
            uid: this.setMan[3],
            childid: childCont[0] * 1,
            ParentType: childCont[1] * 1
          })
        }
      }
      api
        ...
        .then(res => {
          if (res.data.code === 0) {
            this.$emit('update:setVsnackbar', [true, '添加成功', 'success'])
            this.getuser({'uid': this.setMan[3]}, 1)
            return false
          }
          this.$emit('update:setVsnackbar', [true, res.data.msg, 'error'])
        })
