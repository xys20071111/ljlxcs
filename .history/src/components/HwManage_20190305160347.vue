  ...
  <v-layout column wrap>
    ...
    <v-flex>
      <dataTable1 :desserts="desserts" :pagination.sync="pagination" :totalDesserts="totalDesserts" which="hwManage0" 
        :loading="loading" @getclasslist="getHwlist" @clickList="clickListTableItem" 
        @clickjump="jumpToUserClass" :news="news" :clickWhich="clickWhich" :setNew.sync="news">
      </dataTable1>
    </v-flex>
    <v-navigation-drawer
      v-model="drawer"
    ...
    >
      ...
      <v-list class="pa-1">
        ...
        <v-list-tile>
          <v-list-tile-content>
            <v-list-tile-title>作业ID：{{items.hw_id}}</v-list-tile-title>
          </v-list-tile-content>
          <v-spacer></v-spacer>
                ...
                <v-card tile flat>
                  ...
                  <v-list dense>
                    ...
                    <v-layout row wrap mb-2>
                      <Card xl12 lg12 xs12 sm12 md12 d-flex  which="hwManage" :data="[items]" v-if="items" @jumpToUserClass="jumpToUserClass">
                      </Card>
                    </v-layout>
                  </v-list>
                </v-card>
...
export default {
  ...
  methods: {
    ...
    getHwlist (oldOrNo) {
      ...
      let dataObj = {'num': !oldOrNo ? 1 : this.pagination.page, 'size': (this.pagination.rowsPerPage === 1 || !oldOrNo) ? 20 : this.pagination.rowsPerPage}
      this.loading = true
