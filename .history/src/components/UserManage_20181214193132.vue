          ...
          <v-layout row wrap>
            <v-flex sm12 d-flex >
              <v-card tile>
                ...
                <v-list dense>
                  <v-layout row wrap>
                    <v-flex v-for="(item, index) in userInfo" :key="index">
                      <v-list-tile v-for="(value, key) in item" :key="key">
                        <v-list-tile-content class="grey--text">{{value}}</v-list-tile-content>
                        <v-list-tile-content class="align-end" v-if="key === 'gender'">{{ formatGender(items.gender) }}</v-list-tile-content>
                        <v-list-tile-content class="align-end" v-else-if="key === 'user_role'">{{ formatUserRole(items.user_role) }}</v-list-tile-content>
                        <!-- <v-list-tile-content class="align-end" v-else-if="key === 'country'">
                          {{items.country}}
                        </v-list-tile-content> -->
                        <v-list-tile-content class="align-end" v-else>{{ items[key] }}</v-list-tile-content>
                      </v-list-tile>
                    </v-flex>
...
export default {
  ...
  data () {
    return {
      ...
      userInfo: [
        [{user_id: '用户ID：'}, {uname: '昵称：'}, {realname: '真实姓名：'}, {mobile: '手机号：'}],
        [{gender: '性别：'}, {user_role: '身份：'}, {p_id: '推广员ID：'}, {origin: '来源ID：'}],
        [{country: '地区：'}, {in_time: 'in_time：'}, {last_time: 'last_time：'}, {intro: '简介：'}]
      ],
      items: [],
