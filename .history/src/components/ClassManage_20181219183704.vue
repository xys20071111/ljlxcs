  ...
  <v-layout column wrap>
    ...
    <v-list class="pa-1">
      ...
      <v-list-tile avatar>
        <v-list-tile-avatar>
          <v-layout>
              <v-dialog v-model="dialog" max-width="290" :disabled="formatAvatar(items && items.props && items.props.logo_url).indexOf('uLogoNone.jpg') !== -1">
                <img slot="activator" style="width:40px;height:40px" :src="items && items.props && formatAvatar(items.props.logo_url)">
                <v-card>
                  <img :src="items && items.props && formatAvatar(items.props.logo_url)" style="width:290px;height:290px">
                  <v-card-actions>
                    <v-spacer></v-spacer>
                ...
                <template slot="items" slot-scope="props">
                  ...
                  <td class="text-xs-right">
                    <Dialog color="info" :butCont="formatEduRole(props.item.edu_role, props.item.is_owner, props.item.is_stu) === '创建者' ? '不能移除' : '移除'"
                      :quitOrNo = "formatEduRole(props.item.edu_role, props.item.is_owner, props.item.is_stu) === '创建者'" 
