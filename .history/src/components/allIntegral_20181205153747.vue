<template>
  <div>
    ...
    <v-flex class="allinteTitle blue lighten-3 white--text lighten-5--text" d-flex>资产摘要</v-flex>
    <dataTable :headers="header" :desserts="dessert" :loading="load"></dataTable>
    <v-flex class="allinteTitle" mt-3>资产明细</v-flex>
    ...
    getNewData (newPage) {
      ...
      if (!newPage) {
        ...
        axios(this.HOSTCSM, 'asset', dataObj, cb => {
          ...
          this.dessert = handleStatis([cb.data])
        }, 'nodata')
      }
