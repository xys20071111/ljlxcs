<template>
  <div>
    ...
    <dataTable :headers="headers" :desserts="desserts" :totalDesserts="totalDesserts" :loading="loading" :news="news" :setNew.sync="news" @getNew="getNew"></dataTable>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getNew (newPage) {
      ...
      if (newPage.rowsPerPage === -1 && thisDessert.length < this.totalDesserts) {
        this.getNewData(newPage)
      } else if (thisDessert.length < this.totalDesserts && (newPage.page + 1) * newPage.rowsPerPage > thisDessert.length) {
        this.getNewData(newPage)
      }
    },
