        ...
        <v-container fluid grid-list-md>
          <v-layout row wrap>
            ...
            <Card which="class" :data="items.ex.teahcer.ClsList" v-if="items.ex && items.ex.teahcer && items.ex.teahcer.ClsList" @jumpToChildClass="jumpToChildClass">
              <v-flex md12 slot="buttonSlot" slot-scope="{props}">
                <v-list-tile>
                  <Dialog color="info" :butCont="quitOrNo(props.item) ? '创建者不能退出班级' : '退出班级'" :quitOrNo="quitOrNo(props.item)" alert="true" :unbindWho="props.item" which="unbindClass" @unbindClass="setEleFun"></Dialog>
                </v-list-tile>
              </v-flex>
            </Card>
