  ...
  <v-layout column wrap>
    ...
    <v-flex>
      <dataTable :desserts="desserts" which="classSpace0" :loading="loading" @getNew="getNew" :clickWhich="clickWhich"
        :news="news" :setNew.sync="news" @clickList="clickListTableItem" @clickjump = "jumpToUserClass">
      </dataTable> 
    </v-flex>
    <!-- 动态详情 -->
    <v-navigation-drawer
    ...
    <v-list class="pa-1">
      ...
      <v-list-tile>
        <v-list-tile-content>
          <v-list-tile-title>动态ID：{{items.obj_id}}</v-list-tile-title>
        </v-list-tile-content>
        <v-spacer></v-spacer>
          ...
          <v-flex d-flex >
            <v-card tile flat>
              ...
              <v-list dense>
                ...
                <!-- 空间动态详情表格 -->
                <v-layout row wrap>
                  <Card  xl12 lg12 xs12 sm12 md12 d-flex  which="spaceClass" :data="[items]" v-if="items" @jumpToUserClass="jumpToUserClass">
                  </Card>
                </v-layout>
              </v-list>
            </v-card>
...
export default {
  ...
  methods: {
    ...
    // 跳转到用户和班级详情
    jumpToUserClass (userclassid, which, whichEle) {
      if (userclassid) {
        clearTimeout(this.timeout)
        this.timeout = window.setTimeout(() => {
          console.log(this.$parent.$parent.clickRouterName)
          let userclassIndex = this.$router.options.routes.findIndex(value => { return value.name === (whichEle === 'class_id' ? '班级管理' : '用户管理') })
          this.$parent.$parent.clickRouterName({name: whichEle === 'class_id' ? '查询班级' : '查询用户'}, [userclassIndex, 0], userclassid)
        }, 250)
      } else {
        clearTimeout(this.timeout)
      }
