  ...
  methods: {
    ...
    setManFun (manArr) {
      ...
      api
        ...
        .then(res => {
          if (res.data.code === 0) {
            this.$emit('update:setVsnackbar', [true, res.data.msg, 'success'])
            this.$set(this.items, 'uname', manArr[0])
            this.$set(this.items, 'gender', manArr[1])
            this.$set(this.items, 'user_role', manArr[0])
            return false
          }
