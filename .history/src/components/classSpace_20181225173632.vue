...
<div>
  <v-layout column wrap>
    ...
    <v-flex>
      <dataTable1 :desserts="desserts"  which="classSpace0" :loading="loading" >
      </dataTable1> 
    </v-flex>
  ...
  methods: {
    ...
    getspacelist (oldOrNo) {
      ...
      axios('csm.ashx?method=spacedetail&' + dataObj, '', {}, cb => {
        this.logining = false
        cb.data.length < 20 ? (this.news = 'new') : (this.news = 'news')
        if (cb.code === 0) {
          console.log(cb.data)
          this.loading = false
          cb.data.map((val, ind) => {
          ...
          this.desserts = oldOrNo ? cb.data : this.desserts.concat(cb.data)
    ...
    scrollBottom () {
      ...
      this.news === 'news' && this.getspacelist('true')
      this.news = 'new'
    }
  },
  mounted () {
