      ...
      >
        <template slot="items" slot-scope="props">
          ...
          <td style="white-space:nowrap">
            ...
            {{props.item.def_name && props.item.class_name ? '——' + props.item.def_name : props.item.def_name}}
            ({{ props.item.class_id }}) 
          </td>
...
export default {
    ...
    getclassinfo (classid, which) {
      ...
      api
        ...
        .then(res => {
          ...
          let msg = res.data
          if (msg.code === 0) {
            msg.data.cinfo.logo_url = formatAvatar(msg.data.cinfo.logo_url)
            if (which) {
  ...
  activated () {
    ...
    if (this.$route.params.news && this.$route.params.news !== 'news') {
      ...
      this.drawer = true
      this.getclassinfo(this.$route.params.news)
    }
