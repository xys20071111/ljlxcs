      ...
      <v-list class="pt-0" dense>
        ...
        <v-container fluid grid-list-md>
          <v-layout row wrap>
            ...
            <!-- 班级信息 -->
            <Card which="class" :data="items.ex.teahcer.ClsList" v-if="items.ex && items.ex.teahcer && items.ex.teahcer.ClsList" @jumpToChildClass="jumpToChildClass">
            ...
            <Card which="parent" :data="items.ex.stu.Parents" v-if="items.ex && items.ex.stu && items.ex.stu.Parents" @jumpToChildClass="jumpToChildClass">
              <v-flex md12 slot="buttonSlot" slot-scope="{props}">
                <v-list-tile>
                  <Dialog color="info" butCont="解除关系" alert="true" :unbindWho="props.item" which="unbindParent" @unbindParent="setEleFun"></Dialog>
                </v-list-tile>
              </v-flex>
            </Card>
            <!-- 孩子班级信息 -->

            <Card which="stuclass" :data="items.ex.stu.ClsList" v-if="items.ex && items.ex.stu && items.ex.stu.ClsList" @jumpToChildClass="jumpToChildClass">
              <v-flex md12 slot="buttonSlot" slot-scope="{props}">
                <v-list-tile>
  ...
  methods: {
    ...
    getUserList (props) {
      ...
      let params = {
        ...
        param: {
          method: 'getuserlist',
          data: JSON.stringify({'role': !props ? 0 : 4 - props, 'num': this.pagination.page, 'size': this.pagination.rowsPerPage})
        }
      }
      api
        .post(params)
