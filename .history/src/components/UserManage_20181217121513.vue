...
<div>
  <v-layout column wrap>
    ...
    <v-flex>
      ...
      <v-layout row wrap>
        <v-btn v-for="(item, index) in ['ALL', '老师', '家长', '学生']" :key="index" :color="role === (index === 0 ? 0 : 4-index) ? 'success' : 'primary'" depressed @click="getUserList(index === 0 ? 0 : 4-index)">{{item}}</v-btn>
        <v-divider
          class="mx-3"
          inset
          vertical
        ...
        <v-container fluid grid-list-md>
          <v-layout row wrap>
            ...
            <Card which="parent" :data="items.ex.stu.Parents" v-if="items.ex && items.ex.stu && items.ex.stu.Parents" @jumpToChild="jumpToChild">
              <v-flex md12 slot="buttonSlot" slot-scope="{props}">
                <v-list-tile>
                  <Dialog color="info" butCont="解除关系" alert="true" :unbindWho="props.item" which="unbindClassChild" @unbindClass="setEleFun"></Dialog>
                </v-list-tile>
              </v-flex>
            </Card>
            <!-- 孩子班级信息 -->
            ...
            <Card which="stuclass" :data="items.ex.stu.ClsList" v-if="items.ex && items.ex.stu && items.ex.stu.ClsList">
              <!-- <v-flex md12 slot="buttonSlot" slot-scope="{props}">
                <v-list-tile>
                  <Dialog color="info" butCont="退出班级" alert="true" :unbindWho="props.item" which="unbindClassChild" @unbindClass="setEleFun"></Dialog>
                </v-list-tile>
              </v-flex> -->
            </Card>
