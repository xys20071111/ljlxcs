...
<div>
  <v-layout column wrap>
    ...
    <v-flex>
      <dataTable1 :desserts="desserts" :pagination.sync="pagination" :totalDesserts="totalDesserts" which="hwManage0" 
        :loading="loading" @getclasslist="getHwlist" @clickList="clickListTableItem" :news="news" :setNew.sync="news">
      </dataTable1>
      <!-- <v-data-table
      ...
      >
        <template slot="items" slot-scope="props">
          <tr @click.stop="clickListTableItem(props.item)">
            <td class="text-xs-left" width="100">{{ props.item.hw_id }}</td>
            <td class="text-xs-left" width="200">{{ props.item.title }}</td>
            <td class="text-xs-left">{{ props.item.owner }}</td>
            <td class="text-xs-left" style="word-wrap:break-word; max-width:400px">{{ props.item.content }}
            ...
            <td class="text-xs-right">{{ props.item.summary&&props.item.summary.cnt_submit }}</td>
    ...
    <v-list class="pa-1">
      ...
      <v-list-tile>
        <v-list-tile-content>
          <v-list-tile-title>hw_id：{{items.hw_id}}</v-list-tile-title>
        </v-list-tile-content>
        <v-spacer></v-spacer>
...
export default {
  ...
  data () {
    return {
      ...
      pagination: {},
      headers: [
        { text: 'hw_id', value: 'hw_id', align: 'left', sortable: false },
        ...
        { text: '提交作业数', value: 'summary.cnt_submit', align: 'right' },
  ...
  methods: {
    ...
    getHwlist (oldOrNo) {
      ...
      this.loading = true
      axios(this.HOSTCSM, 'gethwlist', dataObj, cb => {
        this.loading = false
        if (cb.code === 0) {
