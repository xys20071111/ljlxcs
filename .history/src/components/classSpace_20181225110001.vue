...
<div>
  <v-layout column wrap>
    ...
    <v-flex>
      ...
      <v-layout row wrap>
        <v-btn color="success" depressed @click="getspacelist()">ALL</v-btn>
        <v-divider
          class="mx-3"
          inset
          vertical
...
export default {
  ...
  methods: {
    ...
    getspacelist (oldOrNo, classId) {
      alert(123)
      this.loading = true
      !oldOrNo && (this.news = 'news')
      let dataObj = {'_id': this.pagination.page}
      classId && (dataObj.class_id = classId)
      axios(this.HOSTCSM, 'spacedetail', dataObj, cb => {
        this.logining = false
        if (cb.code === 0) {
          console.log(cb.data)
          this.loading = false
