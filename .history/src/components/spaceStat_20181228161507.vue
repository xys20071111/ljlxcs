<template>
   <div>
    ...
    <dataTable :which="'spaceStat'" :desserts="desserts" :loading="loading" @clickjump="jumpToChildClass"></dataTable>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getNewList (classOrNo) {
      ...
      axios(this.HOSTCSM, 'getspacesummary', dataObj, cb => {
        ...
        if (!cb.data.length && classOrNo) {
          this.$emit('update:setVsnackbar', [true, '没有数据，请确认争取的班级ID', 'error'])
        }
    ...
    // 跳转到班级
    jumpToChildClass (classid) {
      if (classid) {
        clearTimeout(this.timeout)
        this.timeout = window.setTimeout(() => {
          let classIndex = this.$router.options.routes.findIndex(value => { return value.name === '班级管理' })
          this.$parent.$parent.clickRouterName({name: '查询班级'}, [classIndex, 0], classid)
        }, 200)
      } else {
        clearTimeout(this.timeout)
      }
