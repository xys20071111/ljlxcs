...
<div>
  <v-layout column wrap>
    ...
    <v-flex>
      ...
      <v-layout row wrap>
        <v-btn v-for="(item, index) in ['ALL', '老师', '家长', '学生']" :key="index" :color="role === index ? 'success' : 'primary'" depressed @click="getUserList(index)">{{item}}</v-btn>
        <v-divider
          class="mx-3"
          inset
          vertical
        ...
        <v-container fluid grid-list-md>
          <v-layout row wrap>
            ...
            <Card which="class" :data="items.ex.teahcer.ClsList" v-if="items.ex && items.ex.teahcer && items.ex.teahcer.ClsList">
              <v-flex ms12 slot="buttonSlot" slot-scope="{props}">
                <v-list-tile>
