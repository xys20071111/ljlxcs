  ...
  <v-layout column wrap>
    ...
    <v-list class="pa-1">
      ...
      <v-list-tile avatar>
        ...
        <v-spacer></v-spacer>
          <v-icon v-if="items && items.props.status===200" color="green">gps_fixed</v-icon>
          ...
          <v-layout row wrap>
            ...
            <v-flex xl12 lg12 xs12 sm12 md12 d-flex >
              ...
              <v-toolbar flat color="white" style="align-items:center">
                ...
                <v-spacer></v-spacer>
                <Dialog style="flex:0 0 auto;margin:0" color="green darken-1" butCont="+ 添加成员" alert="true" :unbindWho="items" which="addMember" @addMember="setclass"></Dialog>
              </v-toolbar>
              <v-divider></v-divider>
              <v-data-table
                :headers="headersItem"
              ...
              >
                <template slot="items" slot-scope="props">
                  <td @click="jumpToMan(props.item.user_id)">
                    ...
                    (user_id: <span @dblclick="jumpToMan()">{{ props.item.user_id }}</span>) 
                  </td>
...
export default {
  ...
  methods: {
      ...
      api
        ...
        .then(res => {
          ...
          if (msg.code === 0) {
            ...
            this.items = msg.data
            this.items.props = msg.data.cinfo
    ...
    setclass (props, whichEle) {
      let whichMethod = {quitclass: 'quitclass'}
      let whichData = {}
      whichEle === 'quitclass' && (whichData = {'uid': props.user_id, 'classid': props.class_id})
      whichEle === 'addMember' && (whichData = {'uid': props, 'classid': this.items.class_id})

      if (whichEle === 'addMember' && (!props || isNaN(props * 1))) {
        this.$emit('update:setVsnackbar', [true, '请输入正确的用户ID', 'error'])
        return false
      }
      console.log(whichData, this.items)
      return false
      let params = {
        api: this.HOSTCSM,
        param: {
