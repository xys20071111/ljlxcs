              ...
              >
                <template slot="items" slot-scope="props">
                  <td @click="jumpToMan(props.item.user_id)">
                    ...
                    {{props.item.uname || '未知'}}(<span @dblclick="jumpToMan()">{{ props.item.user_id }}</span>) 
                  ...
                  <td class="text-xs-right">
                    <Dialog color="info" :butCont="formatEduRole(props.item.edu_role, props.item.is_owner, props.item.is_stu) === '创建者' ? '无移除权限' : '移除'"
  ...
  data () {
    return {
      ...
      headersItem: [
        ...
        { text: '昵称', value: 'nick', align: 'left', sortable: false },
