      ...
      <v-list class="pt-0" dense>
        ...
        <v-container fluid grid-list-md>
          <v-layout row wrap>
            ...
            <!-- 孩子信息 -->
            <Card which="child" :data="items.ex.childlist" v-if="items.ex && items.ex.childlist" @jumpToChildClass="jumpToChildClass">
            ...
            <Card which="class" :data="items.ex.teahcer.ClsList" v-if="items.ex && items.ex.teahcer && items.ex.teahcer.ClsList" @jumpToChildClass="jumpToChildClass">
              <v-flex md12 slot="buttonSlot" slot-scope="{props}">
                <v-list-tile>
                  <Dialog color="info" butCont="退出班级" :quitOrNo="quitOrNo(props.item)" alert="true" :unbindWho="props.item" which="unbindClass" @unbindClass="setEleFun"></Dialog>
                </v-list-tile>
              </v-flex>
            </Card>
...
export default {
  ...
  methods: {
    ...
    // 处理是否可以退出班级
    quitOrNo (item) {
      console.log(item, this.items.ex.teacher.cls_owner)
    },
    // 刷新
    getNewData () {
