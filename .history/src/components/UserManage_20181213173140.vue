        ...
        <v-container fluid grid-list-md>
          <v-layout row wrap>
            ...
            <v-flex sm12 d-flex >
              <v-card tile flat>
                <v-list dense>
                  <Dialog color="info" butCont="编辑用户信息" @getManFun="getManFun" :setMan="setMan" which="setMan" @setManFun="setManFun"></Dialog>
                  ...
                  <Dialog v-if="(!items.ex || !items.ex.childlist || items.ex.childlist.length < 3) && items.user_role > 1" color="info"
                    butCont="添加孩子" which="addChild" :setMan="[]" @addManFun="addManFun">
                  </Dialog>
                </v-list>
              </v-card>
            </v-flex>
...
export default {
  ...
  methods: {
    ...
    getManFun () {
      console.log(this.setMan)
      this.setMan = this.setMan
    }
  },
  watch: {
    search () {
