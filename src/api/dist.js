// 没有logo的默认图片
const uLogoNone = require('@/assets/uLogoNone.jpg')
// 性别状态显示转换
...
const formatUserRole = (props) => {
  return ['学生', '家长', '老师'][props - 1] || '未知'
}
// 用户状态和班级状态显示转换
const formatStatus = (props, userOrNo) => {
  let status = '异常'
  status = props === 100 ? '初始状态' : status
  status = props === 200 ? '正常' : status
  status = props === 201 ? '正常-休假' : status
  status = props === 500 ? '封存' : status
  userOrNo && props !== 200 && (status = '异常')
  return status
}
// 用户头像显示转换avatar
const formatAvatar = (props) => {
  props = props || ''
  let temp = !props.includes('https') ? 'https://wxosscdn.ljlx.com/ulogo/compress_' + props : props
  return props ? temp : uLogoNone
}
// 班内身份显示转换
...
// 处理班级名数据
const getStrClass = (cls, def) => {
  let className = (cls && cls.slice(-1) !== '班' && (cls + '班')) || cls
  let classDef = (className && def && className + '--' + def) || className || def || '未知'
  let classDefKh = (className && def && className + '(' + def + ')') || className || def || '未知'
  let classOne = className || def || ''
  return [className, def, classDef, classDefKh, classOne]
}
// 处理用户名和用户昵称数据
const getStrMan = (name, def) => {
  def = (def && def !== name && '--' + def) || ''
  name = name || '未知'
  return name + def
}
